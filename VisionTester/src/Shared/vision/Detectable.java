...
public abstract class Detectable {
    ...
    public Color averageColor(Mat img, ColorSpace imgSpace) {
        ...
        double leftX = coerce(0, img.cols() - 1, left());
        double rightX = coerce(0, img.cols() - 1, right());

        double topY = coerce(0, img.rows() - 1, top());
        double bottomY = coerce(0, img.rows() - 1, bottom());
